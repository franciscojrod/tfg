(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.filterViewModeSelection = {
    attach: function (context, settings) {
      // Go over every data-entity-id value and filter the field view modes.
      $('.field--widget-entity-browser-entity-reference [data-entity-id]', context).each(function() {
  ...
  function ajaxFilterViewModes(entity_type, entity_id, selector) { 
    ...
    $.ajax({
      url: Drupal.url('ajax-entity-view-modes/' + entity_type + '/' + entity_id),
      type: "POST",
      dataType: "json",
